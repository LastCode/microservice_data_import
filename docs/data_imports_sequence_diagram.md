...
此文檔展示 `/api/v1/imports` API 與所有外部系統之間的完整交互時序圖。
...
| 組件 | 類型 | 說明 |
|------|------|------|
| Client | 外部 | 發起 API 請求 (Browser/Postman) |
| FastAPI | 內部 | REST API 服務 (Port 9090) |
| ImportPipeline | 內部 | ETL 流程編排 |
| Connector | 內部 | 文件獲取 (Linux/SFTP) |
| Processor | 內部 | 數據處理 (Cut/Split) |
| Neo4jLoader | 內部 | 數據加載 |
| NAS/SFTP Server | 外部 | 源數據存儲 |
| Neo4j Database | 外部 | 圖數據庫 |
...
| 交互 | 協議 | 端口 | 認證方式 |
|------|------|------|----------|
| Client → FastAPI | HTTP/HTTPS | 9090 | None (內部) |
| LinuxConnector → NAS | NFS/SMB | 445/2049 | Mount 憑證 |
...
| `app/services/processors.py` | 數據處理 (Cut/Split) |
| `app/services/neo4j_loader.py` | Neo4j 加載 |
| `conf/settings.yaml` | 配置 (DB, Server 憑證) |
| `conf/data_map.csv` | Domain 映射 |
